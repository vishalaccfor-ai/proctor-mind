...
import { useExams, Subject, Topic, Question, QuestionOption } from "@/contexts/ExamContext";
...
export default function ExamBuilder() {
  ...
  const addQuestion = () => {
    ...
    const opts: QuestionOption[] = qOpts.map((text) => ({ id: genId(), text: text.trim() }));
    const q: Question = {
      id: genId(), text: qText.trim(), options: opts, correctOptionId: opts[qCorrect].id,
      difficulty: qDifficulty, subjectId: qSubject, topicId: qTopic || topics[0]?.id || "",
    };
  ...
  const handleSave = async () => {
    ...
    try {
      await createExam({
        title: title.trim(), description: description.trim(), duration,
        ...
        createdBy: user?.id || "", isPublished: true,
      });
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex gap-3">
        <Button size="lg" onClick={handleSave} disabled={!title.trim() || questions.length === 0 || saving}>
          <CheckCircle className="h-4 w-4 mr-2" /> {saving ? "Saving..." : "Save Exam"}
        </Button>
